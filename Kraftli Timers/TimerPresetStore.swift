...
class TimerPresetStore {
    ...
    // MARK: - Timer Preset Management

    func addTimerPreset(_ preset: TimerPreset) {
        presets.append(preset)
    }

    func updateTimerPreset(_ updatedPreset: TimerPreset) {
        if let index = presets.firstIndex(where: { $0.id == updatedPreset.id }) {
            presets[index] = updatedPreset
        }
    }

    func deleteTimerPreset(at indices: IndexSet) {
    ...
    func moveTimerPreset(from source: IndexSet, to destination: Int) {
